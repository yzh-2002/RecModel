...
def predict_fn(prefix, num_preds, model, vocab, device):
    ...
    state, cell = model.init_state(batch_size=1)
    for y in prefix[1:]:  # 预热期，更新模型的隐藏状态
        _, state, cell = model(get_input(), state, cell)
        outputs.append(vocab[y])
    for _ in range(num_preds):  # 预测num_preds步
        y, state, cell = model(get_input(), state, cell)
...
def train_epoch(model, train_loader, loss, optimizer, device, use_random_iter):
    ...
    state, cell = None, None
    for X, Y in train_loader:
        if state is None or cell is None or use_random_iter:
            # 在第一次迭代或使用随机抽样时初始化state
            # 顺序分区抽样时，由于相邻batch的序列相邻，故可保留之前的state
            state, cell = model.init_state(batch_size=X.shape[0])
        else:
...
if __name__ == "__main__":
    ...
    num_hiddens = 512
    model = LSTM(len(vocab), num_hiddens).to('cuda')
    num_epochs, lr = 500, 1
