...
from basic.model.seq2seq import Seq2seqEncoder, Seq2seqDecoder
...
def train(model, data_loader, lr, num_epochs, tgt_vocab, device):
    ...
    for epoch in range(num_epochs):
        for batch in data_loader:
            ...
            Y_hat, _, _ = model(X, dec_input, X_valid_len)
...
def predict_fn(model, src_sentence, src_vocab, tgt_vocab, num_steps, device):
    ...
    for _ in range(num_steps):
        ...
        Y, dec_state, state = model.decoder(dec_X, dec_state, state)
